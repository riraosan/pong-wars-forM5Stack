...
void setup(void)
{
    ...
    // if(!canvas.createSprite(320, 120)){
...
void loop(void)
{
    ...
    for (int_fast16_t y = 0; y < img_height; y += sprite_height)
    {
        ...
        for (int i = 0; i < squares.size(); i++)
        {
            for (int j = 0; j < squares[i].size(); j++)
            {
                ...
                if (squares[i][j] == 0)
                {
                    ...
                    canvas[flip].fillRect(i * SQUARE_SIZE, j * SQUARE_SIZE - y, SQUARE_SIZE, SQUARE_SIZE, color);
                }
                if (squares[i][j] == 1)
                {
        ...
        for (int i = 0; i < pong.size(); i++)
        {
            ...
            // ボールを描画
            canvas[flip].fillCircle(pong[i].x, pong[i].y - y, SQUARE_SIZE / 2, color);
        ...
        canvas[flip].pushSprite(&M5.Displays(1), 0, y);
    }

    // 画面の描画処理を終了
    M5.Displays(1).endWrite();
